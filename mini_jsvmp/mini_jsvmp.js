...
// ---- 工具：常量池管理 ----
class ConstPool {
  constructor(){ this.pool = []; this.map = new Map(); }
  indexOf(v){
...
function tokenize(src){
  ...
  while (i < src.length){
    ...
    // 单字符符号
    if ('()+-*/={},;<>'.includes(c)){ toks.push({type:c, v:c}); i++; continue; }

    // 数字
    if (isDigit(c)){
      let j=i; while (j<src.length && isDigit(src[j])) j++;
      toks.push({type:'num', v: Number(src.slice(i,j))}); i=j; continue;
    }
...
function compile(src){
  ...
  function parseStmt(){
    const t = peek().type;
    if (t==='let'){ parseLet(); return; }
    if (t==='print'){ parsePrint(); return; }
    if (t==='function'){ parseFunctionDecl(); return; }
    if (t==='return'){ parseReturn(); return; }
    if (t==='if'){ parseIf(); return; }
    if (t==='while'){ parseWhile(); return; }
    if (t==='{'){ parseBlock(); return; }
  ...
  function parseReturn(){
    eat('return');
    if (peek().type !== ';'){ parseExpr(); } else { // 无值 return
  ...
  function parseExpr(){
    ...
    while (true){
      const t = peek().type;
      if (t==='=='){ eat('=='); parseComparison(); emit(OP.EQ); }
      else if (t==='!='){ eat('!='); parseComparison(); emit(OP.NE); }
      else if (t==='<'){ eat('<'); parseComparison(); emit(OP.LT); }
      else if (t==='>'){ eat('>'); parseComparison(); emit(OP.GT); }
      else if (t==='<='){ eat('<='); parseComparison(); emit(OP.LE); }
      else if (t==='>='){ eat('>='); parseComparison(); emit(OP.GE); }
      else break;
    }
  }
  ...
  function parseComparison(){
    ...
    while (true){
      const t = peek().type;
      if (t==='+'){ eat('+'); parseTerm(); emit(OP.ADD); }
      else if (t==='-'){ eat('-'); parseTerm(); emit(OP.SUB); }
      else break;
    }
  }
  ...
  function parseTerm(){
    ...
    while (true){
      const t = peek().type;
      if (t==='*'){ eat('*'); parseFactor(); emit(OP.MUL); }
      else if (t==='/'){ eat('/'); parseFactor(); emit(OP.DIV); }
      else break;
    }
  }
  ...
  function parseFactor(){
    const t = peek();
    if (t.type==='num'){ eat('num'); emit(OP.PUSH_CONST, cp.indexOf(t.v)); return; }
    if (t.type==='str'){ eat('str'); emit(OP.PUSH_CONST, cp.indexOf(t.v)); return; }
    if (t.type==='('){ eat('('); parseExpr(); eat(')'); return; }
    if (t.type==='id'){
      // 可能是变量或调用
      const idTok = eat('id');
      if (peek().type==='('){
...
class Env {
  ...
  setHere(name, val){ this.vars[name] = val; }
}
...
function runVM(bytecode, consts, functions, hostBuiltins = {}){
  ...
  while (true){
    ...
    switch(op){
      case OP.PUSH_CONST: {
        const idx = code[ip++]; S.push(readConst(idx)); break;
      }
      case OP.LOAD_VAR: {
        const idx = code[ip++]; S.push(loadVarByIdx(idx)); break;
      }
      case OP.STORE_VAR: {
        const idx = code[ip++]; const v = S.pop(); storeVarByIdx(idx, v); break;
      }
      case OP.ADD: { 
      ...
      case OP.SUB: { const b=S.pop(), a=S.pop(); S.push(a-b); break; }
      case OP.MUL: { const b=S.pop(), a=S.pop(); S.push(a*b); break; }
      case OP.DIV: { const b=S.pop(), a=S.pop(); S.push(a/b); break; }
      case OP.EQ: { const b=S.pop(), a=S.pop(); S.push(a == b); break; }
      case OP.NE: { const b=S.pop(), a=S.pop(); S.push(a != b); break; }
      case OP.LT: { const b=S.pop(), a=S.pop(); S.push(a < b); break; }
      case OP.GT: { const b=S.pop(), a=S.pop(); S.push(a > b); break; }
      case OP.LE: { const b=S.pop(), a=S.pop(); S.push(a <= b); break; }
      case OP.GE: { const b=S.pop(), a=S.pop(); S.push(a >= b); break; }
      case OP.PRINT: { console.log(S.pop()); break; }
      case OP.POP: { S.pop(); break; }
...
function runVMHardened(bytecode, consts, functions, hostBuiltins = {}, invMap, seed){
  ...
  while (true){
    ...
    switch(op){
      case OP.PUSH_CONST: {
        const idx = readImm1(); S.push(readConst(idx)); break;
      }
      case OP.LOAD_VAR: {
        const idx = readImm1(); S.push(loadVarByIdx(idx)); break;
      }
      case OP.STORE_VAR: {
        const idx = readImm1(); const v = S.pop(); storeVarByIdx(idx, v); break;
      }
      case OP.ADD: { 
        const b=S.pop(), a=S.pop(); 
        if (typeof a === 'string' || typeof b === 'string') {
      ...
      default: throw new Error('Bad logical opcode '+op+' (from physical '+physOp+')');
    }
  }
}
