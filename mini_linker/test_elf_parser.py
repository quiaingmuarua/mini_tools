...
from mini_linker.mini_read_elf import (
    read_exact, uleb, 
    ELFFile,
    ELFMAG, EI_CLASS, EI_DATA, ELFCLASS64, ELFDATA2LSB
)
...
@pytest.mark.parametrize("class_val,expected", [
    (1, False),  # ELFCLASS32
    (2, True),   # ELFCLASS64
])
def test_class_detection_parametrized(class_val, expected):
    ...
    assert parser.is_64bit() == expected


@pytest.mark.parametrize("data_val,expected", [
    (1, True),   # ELFDATA2LSB
    (2, False),  # ELFDATA2MSB
])
def test_endian_detection_parametrized(data_val, expected):
    ...
    parser = ELFFile(header)
    assert parser.is_little_endian() == expected
